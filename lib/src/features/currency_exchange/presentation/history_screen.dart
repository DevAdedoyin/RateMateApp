...
class _HistoryScreenState extends ConsumerState<HistoryScreen> {
  ...
  int _currentPage = 0; // Current page index
  bool _loading = false; // Loading indicator
  int _apiCallsMade = 0; // Number of API calls made
  int _maxApiCallsPerDay = 6; // Maximum API calls allowed per day
  ...
  Future<void> _loadApiCallCount() async {
    ...
    final lastCallDate = prefs.getString('lastCallDate') ?? '';
    if (today != lastCallDate) {
      // Reset the counter if it's a new day
      await prefs.setString('lastCallDate', today);
      await prefs.setInt('apiCallsMade', 0);
    }
    setState(() {
      _apiCallsMade = prefs.getInt('apiCallsMade') ?? 0;
    });
  }
  ...
  Future<void> _incrementApiCallCount() async {
    ...
    await prefs.setInt('apiCallsMade', _apiCallsMade);
  }

  Future<void> _loadCurrencies() async {
    if (_loading || _apiCallsMade >= _maxApiCallsPerDay) return;
      ...
      child: Column(
        ...
        children: [
            ...
            child: Row(
              ...
              children: [
                TextButton.icon(
                    ...
                    onPressed: () async {
                      if (_apiCallsMade < _maxApiCallsPerDay) {
                        Map<String, String> selectedDateTime = {
                          "date": "",
                          "time": ""
                        };
                        ...
                        _showLimitReachedMessage(context);
                      }
                    },
                    icon: const Icon(
                      Icons.calendar_month,
          ...
          Container(
            margin: const EdgeInsets.all(5),
            child: Text(
              _apiCallsMade < _maxApiCallsPerDay ? "Usage remaining for today: $_apiCallsMade/$_maxApiCallsPerDay" : "You have reached your daily limit. Please try again tomorrow.",
              style: TextStyle(
                  color: _apiCallsMade < _maxApiCallsPerDay
                      ? Colors.green
                      : Colors.red),
            ),
          ),
  ...
  void _showLimitReachedMessage(BuildContext context) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text(
              'You have reached the maximum number of historical currency checks for today. Please try again tomorrow.'),
          actions: <Widget>[
            TextButton(
              child: const Text('OK'),
              onPressed: () {
